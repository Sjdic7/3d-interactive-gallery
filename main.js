...
class GridGallery {
  constructor({ galleryEl }) {
    ...
    this.galleryItems = [...this.gallery.querySelectorAll(".gallery-grid-figure")];

    this.gap = 50;
    this.isDragging = false;

    // if no columns or items, avoid crashes
    if (!this.galleryCols.length || !this.galleryItems.length) {
      console.warn("GridGallery: no cols or items found");
      return;
    }
  ...
  createGrid(scrollX = 0) {
    // place columns horizontally and wrap them
    gsap.set(this.galleryCols, {
      x: (i) => i * this.colsBounds + scrollX,
      modifiers: {
        x: (x) => {
          const num = Number.parseFloat(x);
          const s = gsap.utils.wrap(-this.colsBounds, this.wrapSizeX - this.colsBounds, num);
          return `${s}px`;
        },
      },
    });
  ...
  createGridCols(element, scrollY = 0, index = 0) {
    const items = [...element.querySelectorAll(".gallery-grid-figure")];
    if (!items.length) return;

    const itemBounds = items[0].clientHeight + this.gap;
    const wrapSizeY = itemBounds * items.length;

    gsap.set(items, {
      y: (i) => i * itemBounds + scrollY,
      modifiers: {
        y: (y) => {
          const num = Number.parseFloat(y);
          const s = gsap.utils.wrap(-itemBounds, wrapSizeY - itemBounds, num);
          return `${s}px`;
        },
      },
    });
  ...
  addEvents() {
    // wheel
    window.addEventListener("wheel", this.mouseWheel.bind(this), { passive: true });
    ...
    window.addEventListener("resize", () => {
      this.resize();
      // recalc layout immediately after resize
  ...
  render() {
    ...
    scroll.current.x = this.lerp(scroll.current.x, scroll.target.x, scroll.ease);
    scroll.current.y = this.lerp(scroll.current.y, scroll.target.y, scroll.ease);

    // update wrapped columns and rows
    this.createGrid(scroll.current.x);

    for (let i = 0; i < this.galleryCols.length; i++) {
      this.createGridCols(this.galleryCols[i], scroll.current.y + i * 100, i);
    }

    // speed (useful for parallax)
...
class Plane {
  constructor({ element, scene }) {
    ...
    // default z (previously undefined)
    this.z = 1;

    // create mesh & uniforms first, then add listeners
    this.createMesh();
  ...
  addHoverListeners() {
    // now uniforms exist so it's safe
    this.element.addEventListener("mouseenter", () => {
      gsap.to(this.uniforms.uHover, { value: 1, duration: 0.6, ease: "power3.out" });
    });
    this.element.addEventListener("mouseleave", () => {
      gsap.to(this.uniforms.uHover, { value: 0, duration: 0.6, ease: "power3.out" });
    });
  }

  render() {
    // update sizes and position each frame (keeps mesh aligned with DOM)
    this.getDimension();
    this.mesh.position.set(this.offset.x, this.offset.y, this.z);
    this.mesh.scale.set(this.sizes.x, this.sizes.y, 1);

    this.uniforms.uImageSizes.value = [this.image.naturalWidth || this.sizes.x, this.image.naturalHeight || this.sizes.y];
...
class Gl {
  constructor({ canvas }) {
    ...
    this.planes = [];
    this.createPlanes();
    // do NOT pass args to render; it uses globals (scroll)
    this.render();
  }

  createPlanes() {
    const nodes = [...document.querySelectorAll(".gallery-grid-figure")];
    this.planes = nodes.map((el) => new Plane({ element: el, scene: this.setup.scene }));
  }
...
if (!galleryEl) {
  console.error("Gallery element .gallery-grid not found");
} else {
  ...
  if (!canvas) {
    ...
    // grid and gl both run their own RAF loops
  }
}
